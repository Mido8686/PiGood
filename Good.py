...
class PiGood:
    ...
    def get_next_100_digits(self):
        # Get the next 100 digits of Pi, starting after the "3."
        start = self.index
        ...
        # Include "3." for the first call only, then return only digits after the first set
        if start == 2:  # For the first set, include "3."
            return f"3.{block}"
        else:
            return block
